...
module.exports = async function selectIssue(options) {
  const {repo, owner, project, source = 'project'} = options;
  const getIssues = source === 'project' ? getIssuesFromProject(options) : getIssuesFromRepo(options);
  ...
  const issueNames = issues.map(i => i.title);
  console.log('issueNames', issueNames);
